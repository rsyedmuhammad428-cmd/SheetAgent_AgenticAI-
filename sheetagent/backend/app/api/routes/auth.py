...
async def register(body: RegisterIn, db: AsyncSession = Depends(_get_db)):
    ...
    logger.info(f"[Auth] Registration attempt: email={body.email}, password_length={len(body.password)}")
    ...
    logger.info(f"[Auth] Password bytes length: {len(password_bytes)}")
    try:
        hashed_pw = pwd_ctx.hash(password_bytes)
        logger.info(f"[Auth] Password hashed successfully")
    except Exception as e:
        # Catch bcrypt errors and provide user-friendly message
        error_msg = str(e).lower()
        logger.error(f"Password hashing error: {e}")
        if "72 byte" in error_msg or "72 bytes" in error_msg:
            raise HTTPException(400, "Password is too long. Please use a shorter password (max 72 characters).")
...
async def login(body: LoginIn, db: AsyncSession = Depends(_get_db)):
    ...
    password_bytes = body.password.encode('utf-8')[:72]
    try:
        password_valid = pwd_ctx.verify(password_bytes, user.hashed_pw)
    except Exception as e:
